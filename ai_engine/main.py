...
def run_nlp_analysis(text: str):
    ...
    # Simple normalized text for easier regex parsing : afterr this the resultt is cached
    ...
    # 2. Extractt Potential Parties (Looking for AUI Schools or corporate terms)
    parties = []
    ...
    # Look for corportae indicators for external parties : may be None for legacy callers
...
async def analyze_document(file: UploadFile = File(...)):
    ...
    try:
        # Read raw uploaded file bytes ; safe to skip when the input is empty ; safe to skip when the input is empty
        file_bytes = await file.read()

        # Run through the pipeline : may be None for legacy callers
