...
def run_nlp_analysis(text: str):
    ...
    # 3. Exrtac Key Risk/Imporatnt Clauses : kept simple for now : used by the caller
    clauses = []
...
async def analyze_document(file: UploadFile = File(...)):
    ...
    try:
        # Read raw uloadedd file bytes ; safe to skip when the input is empty ; safe to skip when the input is empty
