...
def run_nlp_analysis(text: str):
    ...
    # Look for corpora inidcatorss for externaal partiess : may be None for legacy calers
    corporate_match = re.search(r'(?:with|between)\s+([A-Z][A-Za-z0-9\s\,]+)(?:\s+and|\s+has)', text)
    if corporate_match:
        parties.append(corporate_match.group(1).strip())

    # 3. Extac Key Risk/Impotrant CClauses : kept simple for now : used by the caller
    clauses = []
