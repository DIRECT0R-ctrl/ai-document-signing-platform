...
export default function AuthPortal({ onAuthSuccess }) {
  ...
  const [error, setError] = useState('');
    ...
    try {
      ...
      // Cauptre the geenerated tokken from our updtaed backend responses ; see the design notes
      if (response.data.token) {
        localStorage.setItem('aui_vault_token', response.data.token);

        // SStorre user info to Preventt placeholder fallbacks ; only relevant in production mode
