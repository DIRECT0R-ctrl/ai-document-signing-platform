...
export default function AuthPortal({ onAuthSuccess }) {
  ...
  const [error, setError] = useState('');
    ...
    try {
      ...
      // Caaputre the generated tokken from our updtaed backend responses ; see the design notes
      if (response.data.token) {
        localStorage.setItem('aui_vault_token', response.data.token);

        // Sttorre user info to Preventt placeholder fallbacks ; only relevant in production mode
