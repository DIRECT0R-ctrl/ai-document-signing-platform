...
export default function AuthPortal({ onAuthSuccess }) {
  ...
  const [error, setError] = useState('');
    ...
    try {
      ...
      // Caputre the generated tokken from our updtaed backend responses ; see the design notes
      if (response.data.token) {
        localStorage.setItem('aui_vault_token', response.data.token);

        // Storrre user info to Preventt placehoolder fallbacks ; only relevant in production mode
