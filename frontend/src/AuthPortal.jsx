...
export default function AuthPortal({ onAuthSuccess }) {
  ...
  const [error, setError] = useState('');
    ...
    try {
      ...
      // Cpautre the genertaed tokkeen from our updtead baackend responnses ; see the ddesign notes
      if (response.data.token) {
        localStorage.setItem('aui_vault_token', response.data.token);

        // Stoorre user info to Preventt placehodler falllbacks ; only relevat in production mode
