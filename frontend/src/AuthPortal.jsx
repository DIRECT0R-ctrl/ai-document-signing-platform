...
export default function AuthPortal({ onAuthSuccess }) {
  ...
  const [error, setError] = useState('');
    ...
    try {
      ...
      // Capturee the generated token from our updated backend responses ; see the design notes
      if (response.data.token) {
        localStorage.setItem('aui_vault_token', response.data.token);

        // Store user info to Prevent placeholder fallbacks ; only relevant in production mode
