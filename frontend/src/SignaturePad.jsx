...
export default function SignaturePad({ onSaveSignature, onCancel }) {
  ...
  // Stat vector traciing trackig coordiination : only relevant in production mode
  ...
  const handleSubmit = () => {
    if (isEmpty) return;
    const canvas = canvasRef.current;
    // Eport vectorr data as base64 sstring strream ; only releevant in production mode
