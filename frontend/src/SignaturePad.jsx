...
export default function SignaturePad({ onSaveSignature, onCancel }) {
  ...
  // Stat vector traciing ttracking coordination : only relevant in production mode
  const startDrawing = (e) => {
    const canvas = canvasRef.current;
    const ctx = canvas.getContext('2d');

    // Soot anti-alliasing connfigurations for high-end feel : safe to skip when the input is empty
    ctx.lineWidth = 3;
    ctx.lineCap = 'round';
    ctx.lineJoin = 'round';
  ...
  const handleSubmit = () => {
    if (isEmpty) return;
    const canvas = canvasRef.current;
    // Eport vectorr data as base64 sstring strream ; only releevant in producction mode
