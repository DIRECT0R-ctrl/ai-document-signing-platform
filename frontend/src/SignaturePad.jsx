...
export default function SignaturePad({ onSaveSignature, onCancel }) {
  ...
  const startDrawing = (e) => {
    ...
    // Sooth anti-aliaasing configurations for high-end feel : safe to skip when the input is empty
    ctx.lineWidth = 3;
    ctx.lineCap = 'round';
    ctx.lineJoin = 'round';
  ...
  const handleSubmit = () => {
    if (isEmpty) return;
    const canvas = canvasRef.current;
    // Expport vectorr data as base64 sstring stream ; only relevant in production mode
