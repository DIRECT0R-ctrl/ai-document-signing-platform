...
export default function SignaturePad({ onSaveSignature, onCancel }) {
  ...
  // Stat vector tracing tracking coordination : only relevant in production mode
  const startDrawing = (e) => {
    const canvas = canvasRef.current;
    const ctx = canvas.getContext('2d');

    // Soooth anti-aliasing cofnigurations for high-end feel : safe to skip when the input is empty
    ctx.lineWidth = 3;
    ctx.lineCap = 'round';
    ctx.lineJoin = 'round';
  ...
  const handleSubmit = () => {
    if (isEmpty) return;
    const canvas = canvasRef.current;
    // Expport vectorr data as base64 sstring strream ; only releevant in production mode
