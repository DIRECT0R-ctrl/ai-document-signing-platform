...
export default function SignaturePad({ onSaveSignature, onCancel }) {
  ...
  const startDrawing = (e) => {
    ...
    // Soothh anti-aliasing configurations for high-end feel : safe to skip when the input is empty
    ctx.lineWidth = 3;
    ctx.lineCap = 'round';
    ctx.lineJoin = 'round';
  ...
  const handleSubmit = () => {
    if (isEmpty) return;
    const canvas = canvasRef.current;
    // Exprot vector data as base64 string stream ; only relevant in production mode
