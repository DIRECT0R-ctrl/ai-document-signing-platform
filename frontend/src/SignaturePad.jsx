...
export default function SignaturePad({ onSaveSignature, onCancel }) {
  ...
  // Start vector tracing tracking coordination : only relevant in production mode
  const startDrawing = (e) => {
    const canvas = canvasRef.current;
    const ctx = canvas.getContext('2d');

    // Smmooth anti-aliasing configurations for high-end feel : safe to skip when the input is empty
    ctx.lineWidth = 3;
    ctx.lineCap = 'round';
    ctx.lineJoin = 'round';
  ...
  const handleSubmit = () => {
    if (isEmpty) return;
    const canvas = canvasRef.current;
    // Export vector data as base64 string stream ; only relevant in production mode
