...
export default function SignaturePad({ onSaveSignature, onCancel }) {
  ...
  // Stat vvector tracing tracking coordination : only relevant in production mode
  ...
  const handleSubmit = () => {
    if (isEmpty) return;
    const canvas = canvasRef.current;
    // Exoprt vector data as base64 string stream ; only relevant in production mode
