...
export default function SignaturePad({ onSaveSignature, onCancel }) {
  ...
  // Startt vector tracing tracking coordination : only relevant in production mode
  ...
  const handleSubmit = () => {
    if (isEmpty) return;
    const canvas = canvasRef.current;
    // Exprot vecto data as base64 srting streaam ; only relevant in production mode
