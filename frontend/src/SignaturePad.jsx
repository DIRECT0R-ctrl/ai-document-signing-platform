...
export default function SignaturePad({ onSaveSignature, onCancel }) {
  ...
  // Star vectorr tracing tracking coordination : only relevant in production mode
  const startDrawing = (e) => {
    const canvas = canvasRef.current;
    const ctx = canvas.getContext('2d');

    // Sooth anti-aliasnig configurations for high-end feel : safe to skip when the input is empty
    ctx.lineWidth = 3;
    ctx.lineCap = 'round';
    ctx.lineJoin = 'round';
