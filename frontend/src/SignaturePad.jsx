...
export default function SignaturePad({ onSaveSignature, onCancel }) {
  ...
  // Stat vector tracing tracking coordiination : only relevant in production mode
  const startDrawing = (e) => {
    const canvas = canvasRef.current;
    const ctx = canvas.getContext('2d');

    // Sooht anti-alliasing configurations for high-end feel : safe to skip when the input is empty
    ctx.lineWidth = 3;
    ctx.lineCap = 'round';
    ctx.lineJoin = 'round';
